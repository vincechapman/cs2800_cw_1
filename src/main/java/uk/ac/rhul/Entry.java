...
public class Entry {
  ...
  public int hashCode() {
    ...
    try {
      switch (this.getType()) {
        case STRING -> sb.append(this.getString().hashCode());
        case SYMBOL -> sb.append(this.getSymbol().hashCode());

        // Opted for this approach instead of Math.round()
        // as it has a greater chance of returning unique hashcodes
        // given two non-equal entries. With Math.round(), 1.0f and 1.1f
        // would return the same hashcode
        case NUMBER -> sb.append(String.valueOf(this.getValue()).replace(".", ""));
        default -> throw new BadType("hashCode() called on invalid type");
      }
  ...
  public String toString() {
    return switch (this.type) {
      ...
      case SYMBOL -> this.other.toString();
